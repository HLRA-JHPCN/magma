...
gemvn_template_device_atomic(
    int m, int n, T alpha,
    const T * __restrict__ A, int lda,
    const T * __restrict__ x, int incx,
    T       * __restrict__ y, int incy)
{
    ...
    {   
        ...
        if (DIM_X >= num_threads) // indicated 1D threads configuration. Shared memory is not needed, reduction is done naturally
        {
            if (ty == 0 && ind < m)
            {
                //y[ind*incy] = alpha*res;
                T tmp = alpha*res;
                #if defined(PRECISION_z)
                magmablas_zatomic_add( &(y[ind*incy]), tmp );
                #elif defined(PRECISION_c)
                magmablas_catomic_add( &(y[ind*incy]), tmp );
                #elif defined(PRECISION_d)
                magmablas_datomic_add( &(y[ind*incy]), tmp );
                #else
                magmablas_satomic_add( &(y[ind*incy]), tmp );
                #endif
            }
        }
        else 
    ...
    T       * __restrict__ y, int incy)
{
    ...
    //for (int col=start; col < (blockIdx.x+1)*TILE_SIZE; col += DIM_Y)// at least 3Gflop/s overhead
    {
        ...
        if ( tx == 0 && col < n) {
            ...
            T tmp = alpha*sdata[0 + ty * DIM_X];
            #if defined(PRECISION_z)
            magmablas_zatomic_add( &(y[col*incy]), tmp );
            #elif defined(PRECISION_c)
            magmablas_catomic_add( &(y[col*incy]), tmp );
            #elif defined(PRECISION_d)
            magmablas_datomic_add( &(y[col*incy]), tmp );
            #else
            magmablas_satomic_add( &(y[col*incy]), tmp );
            #endif
        }
