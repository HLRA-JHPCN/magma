    ...
    T**  y_array, magma_int_t* incy)
{
    ...
    gemvn_template_device_atomic<T, DIM_X, DIM_Y, TILE_SIZE>
        ( my_m, (int)n[batchid], alpha, A_array[batchid], (int)lda[batchid], x_array[batchid], (int)incx[batchid], y_array[batchid], (int)incy[batchid]);
}


/******************************************************************************/
template <typename T, const int DIM_X, const int DIM_Y, const int TILE_SIZE>
void gemvn_template_vbatched_atomic(
    magma_int_t* m, magma_int_t* n, T alpha,
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    gemvn_kernel_vbatched_atomic<T, DIM_X, DIM_Y, TILE_SIZE>
        <<< grid, threads, 0, queue->cuda_stream() >>>                    
        ( m, n, alpha, dA_array, ldda, dx_array, incx, dy_array, incy );
}
    ...
    T**  y_array, magma_int_t* incy)
{
    ...
    gemvc_template_device_atomic<T, DIM_X, DIM_Y, TILE_SIZE, trans>
        ( (int)m[batchid], (int)n[batchid], alpha, A_array[batchid], (int)lda[batchid], x_array[batchid], (int)incx[batchid], y_array[batchid], (int)incy[batchid]);
}


/******************************************************************************/
template <typename T, const int DIM_X, const int DIM_Y, const int TILE_SIZE>
void gemvc_template_vbatched_atomic(
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    {                         
        gemvc_kernel_vbatched_atomic<T, DIM_X, DIM_Y, TILE_SIZE, MagmaConjTrans>
            <<< grid, threads, 0, queue->cuda_stream() >>>                    
            ( m, n, alpha, dA_array, ldda, dx_array, incx, dy_array, incy );        
    }
    else if (trans == MagmaTrans)
    {
        gemvc_kernel_vbatched_atomic<T, DIM_X, DIM_Y, TILE_SIZE, MagmaTrans>
            <<< grid, threads, 0, queue->cuda_stream() >>>                    
            ( m, n, alpha, dA_array, ldda, dx_array, incx, dy_array, incy );       
    }
}
